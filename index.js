...
async function deployCommands() {
  ...
  // Leader does not use slash commands. Publish an empty global command list
  // so old Vehicle Life slash commands are removed from every server.
  await rest.put(
    Routes.applicationCommands(config.clientId),
    { body: [] }
  );

  // Also remove any legacy guild-scoped commands created by older versions.
  if (config.guildId) {
    try {
      await rest.put(
      ...
      console.log(`Cleared legacy guild slash commands for ${config.guildId}.`);
    } catch (err) {
      // If the bot is no longer in that old guild, Discord returns 50001.
      // This must not stop the Leader bot from starting.
      if (err?.code === 50001 || err?.status === 403) {
        console.warn(`Could not clear legacy guild commands for ${config.guildId}: Missing Access (ignored).`);
      } else {
        console.error("Legacy guild command cleanup failed:", err);
      }
    }
  }

  console.log("Global slash commands cleared; Leader uses prefix commands.");
}
...
client.on(
  ...
  async message => {
    ...
    // ?commands works without a missing PDF file.
    if (message.content.trim().toLowerCase() === "?commands") {
      const help = [
        "**LEADER — COMMANDS**",
        "",
        "**Server / Progress**",
        "`?progress` — today's server progress + graphs",
        "`?progress 7` — progress for the last 7 days (2–30)",
        "`?setdailyreportrole @role` — set daily report role",
        "`?setasprogressviewchannel` — set daily report channel",
        "",
        "**Channel Shortcuts**",
        "`?setas name` — save current channel as a shortcut",
        "`?removeas name` — remove a shortcut",
        "`?listas` — list shortcuts",
        "`?m name` — mention a saved channel",
        "",
        "**Community Activity**",
        "`?setmainchannel #channel`",
        "`?activity help` / `on` / `off` / `status` / `test`",
        "`?activity reset` / `cooldown 30m` / `tags 5`",
        "`?activity category gaming` / `category all`",
        "`?activity quiet 12am-7am` / `quiet off`",
        "",
        "**Special Welcome**",
        "`?setspecialwelcomechannel` — set the special welcome channel",
        "`?w` — create the latest-member welcome banner",
        "",
        "**Instagram**",
        "`?insta @user` — view saved Instagram",
        "`?setinsta username` — save Instagram username",
        "`?removeinsta` — remove Instagram username"
      ].join("\n");
      return message.channel.send({ content: help });
    }
...
client.on("guildMemberAdd", member => {
  ...
  specialWelcome.notifyNewMember(member).catch(err => {
    console.error("Special welcome notification error:", err);
  });
});
...
client.on("interactionCreate", async interaction => {
  console.log(
    `INTERACTION RECEIVED: type=${interaction.type} command=${interaction.commandName || ""} customId=${interaction.customId || ""}`
  );

  // Leader uses prefix commands. This also safely handles stale slash
  // commands/buttons left over from the old Vehicle Life bot.
  if (!interaction.isRepliable()) return;

  try {
    if (interaction.isChatInputCommand()) {
      await interaction.reply({
        content: "ℹ️ This Leader bot uses prefix commands. Use `?commands` to see the available commands.",
        ephemeral: true
      });
      return;
    }
    ...
    if (interaction.isButton()) {
      await interaction.reply({
        content: "ℹ️ That button belongs to an old command and is no longer active.",
        ephemeral: true
      });
    }
  } catch (err) {
