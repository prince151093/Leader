...
function modLogChannel(guild) {
  const id = moderationLogConfig[guild.id];
  ...
  async message => {
    ...
    if (lowerContent === "?modlogs" || lowerContent.startsWith("?setmodlogs")) {
      ...
      if (lowerContent === "?modlogs") {
        const id = moderationLogConfig[message.guild.id];
        return message.reply(id ? `🛡️ Moderation logs: <#${id}>` : "🛡️ Moderation logs are not configured.");
      }
      const channel = message.mentions.channels.first();
      if (!channel?.isTextBased()) return message.reply("❌ Use `?setmodlogs #mod-logs`.");
      moderationLogConfig[message.guild.id] = channel.id;
      saveJson(MODLOG_CONFIG_FILE, moderationLogConfig);
      return message.reply(`✅ Full moderation logs configured for ${channel}.`);
    }
...
client.on("guildMemberUpdate", async (oldMember, newMember) => {
  if (oldMember.nickname !== newMember.nickname) {
    await sendModerationLog(newMember.guild, "role", { user: `${newMember}`, action: "Nickname changed", before: oldMember.nickname || "None", after: newMember.nickname || "None" });
  }
});
