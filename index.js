...
async function sendModAlerts(){
  const now = zonedHourMinute();

  for (const guild of client.guilds.cache.values()) {
    const st = getModState(guild.id);
    if (st.alertTime !== now || !st.modAlertChannelId) continue;

    const d = zonedDateKey();
    if (st.lastAlertDate[d]) continue;
    ...
    for (const m of guild.members.cache.values()) {
      ...
      if (st.vcModRoleId && m.roles.cache.has(st.vcModRoleId) && a.vcSeconds < st.minReqVcSeconds) {
        missing.push(`VC: ${formatReportDuration(st.minReqVcSeconds - a.vcSeconds)} remaining`);
      }

      if (st.chatModRoleId && m.roles.cache.has(st.chatModRoleId) && a.messages < st.minReqChat) {
        missing.push(`Chat: ${(st.minReqChat - a.messages).toLocaleString("en-US")} messages remaining`);
      }
    ...
    if (lines.length) {
      const embed = new EmbedBuilder()
        .setTitle("⚠️ Moderator Activity Warning")
        .setDescription(
          `The following moderators have not met their daily activity requirements.` +
          `\n\n${lines.join("\n")}`
        )
        .addFields({
          name: "Daily Requirements",
          value: `🎙️ VC: **${formatReportDuration(st.minReqVcSeconds)}**\n💬 Chat: **${st.minReqChat.toLocaleString("en-US")} messages**`,
          inline: false
        })
        .setFooter({ text: `Leader • ${d} • ${MOD_TIME_ZONE}` });

      await ch.send({ embeds: [embed] }).catch(() => {});
    }

    // Mark today's check as completed so the alert is sent only once per day.
    st.lastAlertDate[d] = true;
    saveModState();
  }
}
