...
client.on("messageCreate", async message => {
  ...
  if (lower.startsWith("?warn")) {
    if(!message.member.permissions.has(PermissionFlagsBits.ModerateMembers))return message.reply("❌ You don't have permission to warn members."); const target=message.mentions.users.first(); if(!target)return message.reply("❌ Please mention a user."); const reason=content.split(/\s+/).slice(2).join(" ")||"No reason provided"; return message.channel.send(`⚠️ ${target} has been warned.\nReason: ${reason}`);
  }
  if (lower.startsWith("?unwarn")) {
    if(!message.member.permissions.has(PermissionFlagsBits.ModerateMembers))return message.reply("❌ You don't have permission to unwarn members."); const target=message.mentions.users.first(); if(!target)return message.reply("❌ Please mention a user."); return message.channel.send(`✅ ${target} has been unwarned.`);
  }
...
client.on("interactionCreate",async interaction=>{
  try{
    ...
    if(interaction.isButton() && interaction.customId==="giveaway_join"){
      const entries=giveawayCommand.giveawayEntries?.get(interaction.message.id); if(!entries)return interaction.reply({content:"❌ This giveaway is no longer active.",ephemeral:true});
      if(entries.has(interaction.user.id))return interaction.reply({content:"⚠️ You are already entered in this giveaway!",ephemeral:true}); entries.add(interaction.user.id);
      const old=interaction.message.embeds[0]; if(old){const desc=(old.description||"").replace(/👥 \*\*Entries:\*\* \d+/,`👥 **Entries:** ${entries.size}`);await interaction.message.edit({embeds:[EmbedBuilder.from(old).setDescription(desc)]}).catch(()=>{});} return interaction.reply({content:"🎉 You have successfully entered the giveaway! Good luck! 🍀",ephemeral:true});
    }
...
client.once("clientReady",async()=>{
  ...
  restoreVoiceSessions(client); for(const guild of client.guilds.cache.values())updatePeakOnline(guild); startDailyLoop();
});
