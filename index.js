...
const PASSWORD_COMMANDS = new Set(["?setmodalert","?setalerttime","?setminreqvc","?setminreqchat","?setvcmod","?setchatmod"]);
...
async function sendModAlerts(){const now=zonedHourMinute();for(const guild of client.guilds.cache.values()){const st=getModState(guild.id);if(st.alertTime!==now||!st.modAlertChannelId)continue;const d=zonedDateKey();if(st.lastAlertDate[d])continue;const ch=guild.channels.cache.get(st.modAlertChannelId);if(!ch?.isTextBased())continue;const lines=[];for(const m of guild.members.cache.values()){if(m.user.bot)continue;const a=activityFor(guild.id,m.id),miss=[];if(st.vcModRoleId&&m.roles.cache.has(st.vcModRoleId)&&a.vcSeconds<st.minReqVcSeconds)miss.push(`VC ${formatReportDuration(st.minReqVcSeconds-a.vcSeconds)} short`);if(st.chatModRoleId&&m.roles.cache.has(st.chatModRoleId)&&a.messages<st.minReqChat)miss.push(`Chat ${st.minReqChat-a.messages} messages short`);if(miss.length)lines.push(`${m} — ${miss.join(", ")}`);}if(lines.length)await ch.send(`🔔 **MOD ACTIVITY ALERT — ${d}**\n${lines.join("\n")}`).catch(()=>{});st.lastAlertDate[d]=true;saveModState();}}
...
client.on(
  ...
  async message => {
    ...
    if (isPrefixCommand) {
      ...
      const passwordRequired = PASSWORD_COMMANDS.has(commandName);

      if (passwordRequired) {
        if (parts[parts.length - 1] !== COMMAND_PASSWORD) {
          const reply = await message.channel.send(`${message.author} ❌ This alert-system command requires the admin password.`).catch(() => null);
    ...
    if (mediaState.mediaChannelId === message.channel.id) {
      if (isPrefixCommand && !isKnownLeaderCommand(content)) {
        const reply = await message.channel.send(`${message.author} ❌ Only Leader commands are allowed in this media-only channel.`).catch(() => null);
        await message.delete().catch(() => {});
        if (reply) setTimeout(() => reply.delete().catch(() => {}), 5000);
        return;
      }
...
/* =========================
    ...
    if (lowerContent.startsWith("?warns")) {
      const target=message.mentions.users.first(); if(!target)return message.reply("❌ Use `?warns @user`.");
