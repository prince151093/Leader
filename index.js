...
function isKnownLeaderCommand(t){return new Set(["?modpdf","?setmedia","?warn","?warns","?unwarn","?setmodalert","?setalerttime","?setminreqvc","?setminreqchat","?setvcmod","?setchatmod","?resetalert","?purge","?setdailyreportrole","?setasprogressviewchannel","?progress","?setas","?removeas","?listas","?m","?lock","?unlock","?activity","?setmainchannel","?setnick","?giverole","?removerole","?mute","?unmute","?addinsta","?removeinsta","?insta"]).has(commandNameOf(t));}
...
/* =========================
    ...
    if (lowerContent === "?resetalert") {
      ...
      if (st.lastAlertDate && st.lastAlertDate[today]) {
        delete st.lastAlertDate[today];
      }
      saveModState();
      return message.reply(`✅ **Today's moderator alert has been reset.**\nThe next scheduled check at **${st.alertTime || "Not set"}** (${MOD_TIME_ZONE}) can send the alert again.`);
    }
