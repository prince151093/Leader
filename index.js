...
async function sendModAlerts(){
  const current = zonedHourMinute();
  const today = zonedDateKey();
  const [ch, cm] = current.split(":").map(Number);

  for (const guild of client.guilds.cache.values()) {
    const st = getModState(guild.id);
    if (!st.modAlertChannelId || st.lastAlertDate[today]) continue;

    const [ah, am] = String(st.alertTime || "07:30").split(":").map(Number);
    const currentMinutes = ch * 60 + cm;
    const alertMinutes = ah * 60 + am;

    // Run once the configured daily time is reached. This also prevents a
    // restart a few seconds/minutes after the configured time from losing the
    // day's alert completely.
    if (currentMinutes < alertMinutes) continue;

    const channel = guild.channels.cache.get(st.modAlertChannelId);
    if (!channel?.isTextBased()) continue;

    // Capture voice time right before evaluating the requirements so a mod
    // currently in VC gets credit for their latest active minute.
    await updateTrackedModVc();

    // Make sure role members are available even on servers where not every
    // member is currently cached.
    ...
    for (const member of guild.members.cache.values()) {
      ...
      const activity = activityFor(guild.id, member.id);
      const isVcMod = !!st.vcModRoleId && member.roles.cache.has(st.vcModRoleId);
      const isChatMod = !!st.chatModRoleId && member.roles.cache.has(st.chatModRoleId);

      // VC moderators are judged ONLY on VC time.
      if (isVcMod && activity.vcSeconds < st.minReqVcSeconds) {
        const remaining = formatReportDuration(st.minReqVcSeconds - activity.vcSeconds);
        vcLines.push(`• ${member} — **${remaining}** VC time remaining`);
        vcMentions.push(member.id);
      }

      // Chat moderators are judged ONLY on chat messages.
      if (isChatMod && activity.messages < st.minReqChat) {
        const remaining = (st.minReqChat - activity.messages).toLocaleString("en-US");
        chatLines.push(`• ${member} — **${remaining}** messages remaining`);
        chatMentions.push(member.id);
      }
    }

    // Do not send an empty warning. If everybody met their role-specific
    // requirement, there is nothing to warn about.
    if (!vcLines.length && !chatLines.length) {
      st.lastAlertDate[today] = true;
      saveModState();
      continue;
    }
    ...
    if (vcLines.length) {
      sections.push(
        `🎙️ **VC MODERATORS**\n` +
        `Required: **${formatReportDuration(st.minReqVcSeconds)}**\n` +
        vcLines.join("\n")
      );
    }
    if (chatLines.length) {
      sections.push(
        `💬 **CHAT MODERATORS**\n` +
        `Required: **${st.minReqChat.toLocaleString("en-US")} messages**\n` +
        chatLines.join("\n")
      );
    }

    const allMentions = [...new Set([...vcMentions, ...chatMentions])];
    const embed = new EmbedBuilder()
      .setTitle("⚠️ Daily Moderator Activity Alert")
      .setDescription(
        `Some moderators have not completed their **role-specific daily requirement**.\n\n` +
        sections.join("\n\n")
      )
      .setFooter({ text: `Leader • ${today} • ${MOD_TIME_ZONE}` })
      .setTimestamp(new Date());

    const mentionText = allMentions.map(id => `<@${id}>`).join(" ");
    ...
    const sent = await channel.send({
      ...
      console.error(`Could not send moderator activity alert in ${guild.id}:`, err);
      return null;
    });

    // Only mark the day as completed if Discord accepted the alert.
    if (sent) {
      st.lastAlertDate[today] = true;
      saveModState();
    }
...
setInterval(()=>updateTrackedModVc().catch(console.error),60000).unref?.();
setInterval(()=>sendModAlerts().catch(console.error),15000).unref?.();
setInterval(()=>scanMediaChannels().catch(console.error),60000).unref?.();
