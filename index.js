...
const AFK_FILE = path.join(__dirname, "afk-data.json");
let afkUsers = {};
...
function loadAfkUsers() {
  try {
    if (fs.existsSync(AFK_FILE)) {
      ...
      afkUsers = data && typeof data === "object" ? data : {};
    }
  } catch (err) {
    console.error("AFK data load error:", err);
    afkUsers = {};
  }
}
...
function saveAfkUsers() {
  try {
    ...
    console.error("AFK data save error:", err);
  }
}
...
function getAfk(guildId, userId) {
  return afkUsers[afkKey(guildId, userId)] || null;
}

function setAfk(guildId, userId, reason) {
  afkUsers[afkKey(guildId, userId)] = {
    reason: reason || "AFK",
    since: Date.now()
  };
  saveAfkUsers();
...
function clearAfk(guildId, userId) {
  const key = afkKey(guildId, userId);
  if (!afkUsers[key]) return false;
  delete afkUsers[key];
  saveAfkUsers();
  return true;
}
...
client.on(
  ...
  async message => {
    ...
    if (await specialWelcome.handleMessage(message)) {
      return;
    }

    // AFK: remove the author's status when they return, then notify when an AFK member is mentioned.
    const afkEntry = getAfk(message.guild.id, message.author.id);
    const isAfkCommand = /^\?afk(?:\s|$)/i.test(message.content.trim());
    ...
    if (!isAfkCommand && message.mentions.users.size) {
      const notices = [];
      for (const [, mentionedUser] of message.mentions.users) {
        const entry = getAfk(message.guild.id, mentionedUser.id);
        if (entry) {
          notices.push(`💤 **${mentionedUser.displayName}** is AFK: **${entry.reason}** — ${formatAfkDuration(entry.since)}.`);
        }
      }
      if (notices.length) {
        await message.reply(notices.slice(0, 5).join("\n")).catch(() => {});
      }
    }

    // Show the Leader command guide without requiring a separate PDF file.
    if (message.content.trim().toLowerCase() === "?commands") {
      return message.reply(
        "📘 **Leader Command Guide**\\n\\n" +
        "`?activity` — Server activity controls\\n" +
        "`?setas <name>` — Set a channel shortcut\\n" +
        "`?m <name>` — Mention a saved channel shortcut\\n" +
        "`?purge <amount>` — Delete recent messages\\n" +
        "`?warn @user [reason]` — Warn a member\\n" +
        "`?unwarn @user [reason]` — Remove a warning\\n" +
        "`?giverole @user @role` — Give a role\\n" +
        "`?removerole @user @role` — Remove a role\\n" +
        "`?mute @user` — Mute a member\\n" +
        "`?instagram` — Instagram profile commands\\n" +
        "`/giveaway` — Create/manage a giveaway"
      );
    }
    ...
    if (lowerContent === "?afk" || lowerContent.startsWith("?afk ")) {
      ...
      if (reason.toLowerCase() === "off") {
        ...
        return message.reply("ℹ️ You are not currently AFK.");
      }
      const cleanReason = reason || "AFK";
      setAfk(message.guild.id, message.author.id, cleanReason.slice(0, 200));
...
client.on(
  ...
  async interaction => {
    ...
    try {
      ...
      // Leader uses message commands plus /giveaway.
      return;
