...
function activityHelp() {
  return [
    ...
    "`?vclock` — lock the current voice channel",
    "`?vcunlock` — unlock the current voice channel",
...
async function handleActivityCommand(message) {
  ...
  if (lower === "?vclock" || lower === "?vcunlock") {
    if (!manage) return message.reply("❌ You need **Manage Server** permission.");
    const locking = lower === "?vclock";
    if (!channelIsVoice(message.channel)) return message.reply("❌ Use `?vclock` or `?vcunlock` inside a voice channel.");
    try {
      await message.channel.permissionOverwrites.edit(message.guild.roles.everyone, {Connect: locking ? false : null, Speak: locking ? false : null}, {reason: locking ? `Voice channel locked by ${message.author.tag}` : `Voice channel unlocked by ${message.author.tag}`});
      return message.reply(locking ? "🔒 **Voice channel locked.** Anyone entering will be disconnected." : "🔓 **Voice channel unlocked.**");
    } catch(err) { console.error("VC lock error:",err); return message.reply("❌ I couldn't change the voice channel lock."); }
  }
...
function channelIsVoice(channel) { return channel && [ChannelType.GuildVoice, ChannelType.GuildStageVoice].includes(channel.type); }
...
client.on("voiceStateUpdate",async(oldState,newState)=>{
  try{
    ...
    if (newState.channelId && newState.channel?.permissionOverwrites.cache.get(newState.guild.roles.everyone.id)?.deny.has(PermissionFlagsBits.Connect)) { await newState.disconnect("Voice channel is locked").catch(()=>{}); return; }
